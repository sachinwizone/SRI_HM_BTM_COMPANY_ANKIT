...
export class ObjectStorageService {
  ...
  async getClientDocumentUploadURL(clientId: string, documentType: string): Promise<string> {
    ...
    // Ensure documentType is in camelCase for consistent paths
  ...
  async findClientDocumentFile(clientId: string, documentType: string): Promise<File | null> {
    const privateObjectDir = this.getPrivateObjectDir();
    if (!privateObjectDir) {
      console.error(`❌ Private object dir not set`);
      return null;
    }

    console.log(`🔍 Finding document: clientId=${clientId}, documentType=${documentType}`);
    ...
    try {
      ...
      console.log(`🔍 Trying exact path: ${newPath}`);
      const { bucketName, objectName } = parseObjectPath(newPath);
      console.log(`🔍 Bucket: ${bucketName}, Object: ${objectName}`);
      const bucket = objectStorageClient.bucket(bucketName);
      const file = bucket.file(objectName);
      const [exists] = await file.exists();
      console.log(`🔍 File exists at exact path: ${exists}`);
      if (exists) {
        console.log(`✅ Found file at exact path: ${newPath}`);
        return file;
      }
    } catch (error) {
      console.error(`❌ Error checking exact path:`, error);
    ...
    try {
      ...
      console.log(`🔍 Listing files in uploads directory: ${uploadsPath}`);
      ...
      for (const file of files) {
        console.log(`🔍 Checking file: ${file.name} for clientId: ${clientId}, documentType: ${documentType}`);
